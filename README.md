...
이 문서는 현재 로컬 파일 구조와 import/entrypoint 확인 결과를 기준으로 작성했다. 실제 전처리 실행, 외부 API 호출, 모델 다운로드/학습/추론, DB DDL/DML, 크롤링, 주문 실행은 수행하지 않았다.
...
- 뉴스 전처리 후보
  ...
  - `preprocessor_v1.py`: `sentence_transformers`, `transformers`, `KeyBERT`를 사용하는 legacy 또는 모델 기반 실험 후보. 모델 다운로드/추론 가능성이 있으므로 문서화 작업 중 실행하지 않는다.
  - `test_pre_news_analysis.py`: 테스트 또는 로컬 검증 후보로만 취급한다.
- 증권사 리포트 전처리 후보
...
- total feature 전처리 후보
  ...
  - `pre_total_daily_feature.py`: 일부 daily feature를 결합해 `pre_total_daily_feature`를 재구성하는 후보. 내부에 truncate 성격의 DB 작업이 있으므로 실행 전 영향 범위 확인이 필요하다.
- 보조 후보
  - `interest_get_holidays.py`: 외부 holiday API를 조회하는 휴일 helper 후보. 외부 API 호출이 있으므로 문서화 작업 중 실행하지 않는다.
  - `interest_pool.py`: 뉴스 분석 결과를 기반으로 관심 pool 후보를 저장하는 스크립트.
- 로컬 산출물 후보
  - `__pycache__/`: Python bytecode cache 산출물로 운영 소스로 단정하지 않는다.
...
각 스크립트는 독립 실행형 `run()` 또는 `main()` entrypoint를 가진 파일이 많다. 다만 실행 시 DB 연결, upsert, truncate 성격의 작업, 외부 holiday API 호출, 모델 다운로드/추론이 발생할 수 있으므로 운영 환경에서만 의도적으로 실행해야 한다.
...
현재 스크립트들은 DB 접속정보, 처리 버전, 외부 holiday API URL, 모델명 또는 로컬 환경 설정을 사용할 수 있다.
...
- 모델 기반 legacy 스크립트의 모델명과 cache 경로
...
외부 API 호출, 모델 다운로드/학습/추론, DB DDL/DML은 운영 영향이 있으므로 분석 또는 문서 작업 중에는 실행하지 않는다.
...
- test/debug/cache/model/output dump 파일은 운영 소스로 단정하지 않고 후보 또는 로컬 산출물로만 표현
